...
class ABCNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.upsample(feat_out, x.size()[2:], mode='bilinear', align_corners=True), F.upsample(feat_out16, x.size()[2:], mode='bilinear', align_corners=True), \
               F.upsample(feat_out32, x.size()[2:], mode='bilinear', align_corners=True)
